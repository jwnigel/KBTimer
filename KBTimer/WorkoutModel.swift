...
class WorkoutModel {
    ...
    init(order: Int, sets: [WorkoutSet], rests: [Int], completed: Bool = false) {
        self.order = order
        self.sets = sets
        self.rests = rests
...
//        container.mainContext.insert(WorkoutModel(order: 9, sets: [7], rests: [], completed: false))
        container.mainContext.insert(WorkoutModel(order: 10, 
                                                  sets: [WorkoutSet(minutes: 4, completed: true),
                                                         WorkoutSet(minutes: 3, completed: true),
                                                                    WorkoutSet(minutes: 2, completed: false),
                                                                    WorkoutSet(minutes: 1, completed: false)],
                                                  rests: [5, 5],
                                                  completed: false))
